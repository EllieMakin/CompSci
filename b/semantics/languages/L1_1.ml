  ...
  | Op (e1,opr,e2) -> 
      ...
      | (e1,opr,e2) -> (                                               
          if (is_value e2) then 
            ...
            | Some (e1',s') -> Some (Op(e1',opr,e2),s')     (* (op2) *)
            | None -> None )                           
          else 
...
let e = Seq(Seq(
  ...
  While(Seq(
    ...
    Op(Deref("l1"), GTEQ, Integer(1))
  ),
    Seq(Seq(Seq(
      Assign("l3", Deref("l2")),
      Assign("l4", Integer(1))),
      While(Seq(
        Assign("l4", Op(Deref("l4"), Plus, Integer(1))),
        Op(Deref("l1"), GTEQ, Deref("l4"))
      ),
        Assign("l3", Op(Deref("l3"), Plus, Deref("l2")))
      )),
      Assign("l2", Deref("l3"))
    )
  )
